...
class RiskIndicator :
    ...
    def with_apk(self, andro_apk) :
      if andro_apk.is_valid_APK() :
        d = dvm.DalvikVMFormat( andro_apk.get_dex() )
        dx = analysis.uVMAnalysis( d )

        return self.with_apk_direct(andro_apk, d, dx)
      return {}
 
    def with_apk_direct(self, andro_apk, d, dx) :
      res = {}
      for i in self.risk_analysis_obj :
        res[ i.get_name() ] = i.with_apk( andro_apk, d, dx )
      return res
...
#     export_system( SYSTEM, "./output" )
  ...
  def with_apk(self, andro_apk, d, dx) :
    """
      @param andro_apk : an L{APK} object
    ...
    self.__eval_risk_perm( andro_apk.get_details_permissions(), risks )

    self.__eval_risk_dyn( dx, risks )
    self.__eval_risk_bin( andro_apk.get_files_types(), risks )
...
class RedFlags :
    ...
    def with_apk(self, andro_apk, d, dx) :
      ...
      self.analyze_apk( andro_apk, flags["APK"] )
      self.analyze_axml( andro_apk, flags["PERM"] )
    ...
    def analyze_apk(self, a, flags) :
      ...
      andro_apk = [ re.compile("Android application package file"), "APK" ]
      ...
      regexp = [ elf_executable, lib_elf, andro_apk, dex_file, script_file, zip_file ]
